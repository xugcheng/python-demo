...
def prepare_df(df):
    df['校内天线信号强度'] = calculate(df['校内天线信号强度'])
    df['校外天线信号强度'] = calculate(df['校外天线信号强度'])
    df['接收时间'] = df['接收时间'].apply(lambda x:
                                  datetime.datetime.strptime(str(x), '%Y-%m-%d %H:%M:%S'))
    df['新信号强度差值'] = df['校内天线信号强度'] - df['校外天线信号强度']
    df_count = df['手表编号'].value_counts().sort_index(ascending=True)
    return df, df_count
